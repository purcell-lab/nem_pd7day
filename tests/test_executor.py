"""
Tests for executor.py and the clients' use of it: CPU-bound decompression and
CSV parsing stay off the event loop.
...
Run with:  python -m pytest tests/test_executor.py -v
"""
...
_nem_time, _executor_mod, _client_mod = load_chain("nem_time", "executor", "pd7day_client")

run_in_executor = _executor_mod.run_in_executor
PD7DayClient = _client_mod.PD7DayClient

PD7DAY_LISTING = b'<a href="/Reports/Current/PD7Day/PUBLIC_PD7DAY_1.zip">z</a>'


# ── Fixtures ─────────────────────────────────────────────────────────────────

def _minimal_csv() -> bytes:
    """One CASESOLUTION and one PRICESOLUTION row, in the column layout
    asserted in test_pd7day_client.py. Dates are relative so the fixture
    never goes stale."""
...
def _pd7day_client() -> PD7DayClient:
    return PD7DayClient(_FakeSession(PD7DAY_LISTING, make_zip(_minimal_csv())))
...
def test_pd7day_unzip_and_parse_run_off_the_loop():
    ...
    async def scenario():
        ...
        with patch.object(_client_mod, "_parse_all_tables", spy_parse), \
    ...
    assert seen["unzip"] != seen["loop"], "ZIP decompression ran on the event loop thread"
    assert seen["parse"] != seen["loop"], "CSV parsing ran on the event loop thread"
...
def test_event_loop_stays_responsive_during_parse():
    ...
    async def scenario():
        ...
        try:
            with patch.object(_client_mod, "_parse_all_tables", slow_parse):
                await _pd7day_client().fetch_all(["QLD1"])
        finally:
            beat.cancel()
            try:
    ...
    assert ticks >= 10, (
        f"event loop only ticked {ticks} times during a {BLOCK_S}s parse; the work appears to be running on the loop"
    )
...
def test_stpasa_extract_and_parse_run_off_the_loop():
    ...
    inner = make_zip(b"C,NEMP.WORLD,STPASA,AEMO\n", "PUBLIC_STPASA_X.CSV")
    payload = make_zip(inner, "PUBLIC_STPASA_X.zip")
    listing = b'<a href="/Reports/Current/Short_Term_PASA_Reports/PUBLIC_STPASA_1.zip">z</a>'
...
@pytest.mark.parametrize("module_name", ["pd7day_client.py", "stpasa_client.py", "tradingis_client.py"])
def test_no_zipfile_construction_inside_async_functions(module_name):
    ...
    """
    with open(os.path.join(PKG_DIR, module_name), encoding="utf-8") as fh:
    ...
    assert not offenders, (
        f"{module_name}: zipfile used directly inside an async function at line(s) {offenders}; "
        "move it to a sync helper run via run_in_executor()"
    )

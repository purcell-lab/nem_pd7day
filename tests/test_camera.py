...
* Parity with the price forecast sensor, issue #80. The chart and the sensor
  ...
  directly with the gas and QNI covariates only, no ``stpasa_features`` and no
...
SPIKE_QNI_THRESHOLD_MW = _const_mod.SPIKE_QNI_THRESHOLD_MW
...
class StubCoordinator:
    ...
    ``qni_mw`` feed the spike covariate gate; None leaves that covariate out of
...
def make_camera(values, gas_tj=200.0, qni_mw=-500.0):
    ...
    _sensor, camera, _coord, _store = make_pair(periods, gas_tj=gas_tj, qni_mw=qni_mw)
    return camera
...
def test_spike_credible_is_the_covariate_gate_and_keeps_its_tri_state(gas, qni, expected):
    ...
    callout. The gate is gas above 150 TJ and QNI below -300 MW, both together.
    ...
    camera = make_camera([0.10, 12.0, 0.08], gas_tj=gas, qni_mw=qni)
    entries = camera._build_forecast_data()
    ...
    assert spike[0]["spike_credible"] is expected, (
        f"gas={gas} qni={qni} gave {spike[0]['spike_credible']!r}, expected {expected!r}"
    )

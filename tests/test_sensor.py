...
def test_native_value_passes_covariates_to_the_store():
    """The covariate gate can only fire if gas and the network covariate reach apply_to_price (#66, #177)."""
    def _apply(
        raw, h, hour, *,
    ...
    # The spike gate reads every link the region imports across, in that
    # region's direction, so both QLD1 links have to be present. The second
    # interval sits at full capability: it is what makes the run median the
    # target interval is judged against meaningful (#176).
    def _flow(time_iso, exportlimit):
        return types.SimpleNamespace(time=time_iso, mwflow=400.0, exportlimit=exportlimit, importlimit=-1000.0)

    next_start = nem_iso(datetime.fromisoformat(period.time) + timedelta(hours=1))
    ic_forecast = [_flow(period.time, 50.0), _flow(next_start, 1000.0)]
    market_summary = MagicMock()
    market_summary.forecast = [MagicMock(nemtime=period.time, value_tj=100.0)]
    sensor.coordinator.data.interconnectors = {
        ic_id: types.SimpleNamespace(forecast=list(ic_forecast))
        for ic_id in _const_mod.REGION_INTERCONNECTOR_SIGN["QLD1"]
    }
